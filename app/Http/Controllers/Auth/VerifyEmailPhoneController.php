...
class VerifyEmailPhoneController extends Controller
{
    ...
    public function store(Request $request, $type = 'email')
    {
        ...
        $code = ($type == 'email')
            ? $request->user()->email_verify_code
            : ($type == 'phone'
                ? $request->user()->phone_verify_code
                : null
            );

        // check if it has not expired: the time is 30 minutes and that the code is valid
        $last_attempt = ($request->user()->hasVerifiedPhone() && $request->user()->last_attempt === null)
            ? $request->user()->phone_verified_at
            : $request->user()->last_attempt;
