...
class SendCode extends Notification implements ShouldQueue
{
    ...
    public function via($notifiable)
    {
        ...
        return collect($channels)->filter(
            fn ($ch) => $this->type !== 'verify-phone' || $ch !== 'mail'
        )->filter(
            fn ($ch) => $this->type !== 'verify' || $ch !== TwilioChannel::class
        )->toArray();
    }
    ...
    public function toMail($notifiable)
    {
        $message = [
            'reset' => [
                ...
                'message_line2' => __('This password reset code will expire in 30 minutes.', [
            ...
            'verify' => [
                ...
                'message_line2' => __('This verification code will expire in :0 minutes.', [
                    config('settings.token_lifespan', 30)
                ]),
    ...
    {
        $message = [
            'reset' => __("Use this code :0 to reset your :1 password, It expires in :2 minutes.", [
                $this->token,
                config('settings.site_name'),
                config('settings.token_lifespan', 30)
            ]),
            'verify-phone' => __("use this code :0 to verify your :1 phone number, It expires in :2 minutes.", [
                $this->token,
                config('settings.site_name'),
                config('settings.token_lifespan', 30)
            ]),
